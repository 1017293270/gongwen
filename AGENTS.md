...
项目默认不使用 Superpowers、多 Agent 或子 Agent 协同。只有当用户在当前任务中明确说“使用 superpowers”“使用多 Agent”“派发子 Agent”“按某个 superpowers skill 执行”等指令时，才允许启用这些流程。

当用户要求“轻量”“少测试”“先做一个能用版本”“继续直接修”时，必须采用单 Agent、小步、少改、真实路径优先的方式；禁止隐性调用 Superpowers，禁止为了流程完整而扩大功能范围。

如果确实被用户明确要求使用多 Agent，必须先拆边界再并行，避免多个 Agent 同时改同一组文件造成互相覆盖。
