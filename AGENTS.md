...
P10D T15 预览刷新与导出状态前端已落地：工作台右栏新增真实预览状态面板，状态覆盖当前、待刷新、生成中、失败和未选择模板；正文编辑、段落生成、局部建议采纳、模板切换和节点格式保存/恢复会标记真实预览待刷新。模板解析工作台继续使用 `POST /api/templates/versions/{versionId}/render-preview` 渲染模板原稿；起草工作台刷新真实预览会先保存当前草稿，再调用 `POST /api/drafts/{draftId}/render-preview`，由后端用当前草稿生成临时 DOCX 并经 LibreOffice 渲染为真实页面图。后端导出阻断消息继续通过工作台导出状态和 Toast 展示，成功导出下载与导出记录行为保持 P11 既有路径。

P10D T15 后续修正：工作台中间预览区打开草稿时会读取当前模板版本已有的 LibreOffice 渲染预览，存在 `READY` 页面时默认展示真实渲染页图，并提供“结构编辑”切换以保留原有节点编辑、AI、格式覆盖和导出功能；没有可用真实预览时继续回退到结构化编辑预览，用户仍可切到真实预览状态并从中间区域触发刷新。后续修正补齐草稿级真实预览：`document_render_preview` 增加可选 `draft_id`（V19），当前草稿预览记录与模板原稿预览隔离，草稿预览不写入导出记录。
...
P10E DOCX 完整结构事实与无占位符范文套版实施计划已新增：`docs/superpowers/plans/2026-05-31-docx-complete-structure-template-pipeline.md`。该阶段不推翻 P10D，而是把 `DocumentStructureProfile` 从 `TemplateProfile.structures` 派生的浅层结构升级为 fact-first OOXML 事实层；语义角色改为可校正建议；`DraftNode` 初始化必须按源节点文本生成，不能再复制首个 `BODY_PARAGRAPH`；无占位符 `REFERENCE_DOCUMENT` 导出应走原 DOCX 节点原位替换，而不是清空并重组正文。P10E T18-T27 已落地并收口：上传模板会持久化 `document-structure-v2` fact nodes（正文段落、表格段落、页眉页脚、位置、runs、编号事实），`DocumentSemanticSuggester` 在事实层上给出标题、主送、日期、正文、正文标题、发文机关、文号等建议，正文开始后不再因“单位/机关/号”等词误判为 `UNIT`/`META`；`DraftNode.initializeNodes` 对已有节点保持非破坏性，首次初始化优先使用每个源节点自己的文本，并保留所有非忽略映射节点，`UNKNOWN` 节点以 `NEEDS_REVIEW` 保留源文本，固定节点以 `LOCKED` 保留源文本，避免未编辑时丢副标题、表格、页眉页脚等原稿事实；显式 `POST /api/drafts/{draftId}/nodes/reinitialize` 会按当前发布映射重建，默认“从原稿重建结构”覆盖旧节点，另有“保留编辑重建”入口保留用户已编辑节点；前端模板解析工作台已拆到 `frontend/src/components/template/**`，完整展示 fact tree，分离事实节点类型和语义建议，支持选中节点批量标为正文、一级标题、固定文本或忽略，并继续复用保存草稿和发布映射 API；工作台已拆出 `WorkbenchStructureTree` 和 `WorkbenchPreview`，纸面区明确标为结构化编辑预览，并按持久化节点源顺序渲染标题、日期、主送、正文、固定文本、页眉页脚等节点，真实原貌仍以 LibreOffice 渲染预览作为视觉准绳；后端导出新增 `DocxNodeLocator` / `DocxNodeReplacementRenderer`，无占位符 `REFERENCE_DOCUMENT` / `OFFICIAL_DOCUMENT` 在存在映射节点时走原 DOCX 节点原位替换，保留占位符模板旧路径，导出记录通过 Flyway `V17` 新增 `export_strategy` 追踪 `PLACEHOLDER_REPLACEMENT`、`ORIGINAL_NODE_REPLACEMENT` 或 `GENERATED_SNAPSHOT`；T25 新增 `DocxCompleteStructurePipelineTest` 覆盖 speech reference 上传、结构事实、语义建议、发布映射、DraftNode 初始化去重、原 DOCX 替换导出和本机 LibreOffice CLI PDF 烟测；T27 针对真实样本反馈补齐“原稿、工作台结构化预览、未修改导出”严格对应的回归，并在 `docs/DOCX_ROUNDTRIP_TEST_CASES.md` 固化无编辑闭环、单节点编辑、忽略节点和工作台预览顺序四类测试用例。
...
- 部门管理删除为物理删除，仅允许删除没有子部门、账号或业务数据引用的空部门；前端必须二次确认，后端继续由系统管理员权限保护并返回稳定错误码。
- 文种管理能力保留 CRUD API，但前端不再作为独立侧边栏模块；新增和删除文种入口下沉到草稿列表和模板管理的文种文件夹页，新增共用同一个创建弹窗，删除使用卡片右上角垃圾桶按钮并二次确认。文种、草稿、模板和导出记录已带 `created_by` / `department_id` 归属字段或索引预留。
- 当前后端已对草稿读取、保存、删除、模板列表/上传/删除、模板版本查询、文种列表/创建/更新/删除接入当前用户过滤：系统管理员可看全部，普通用户优先只看系统内置或自己创建的数据。
- 导出记录列表和历史文件下载已按当前用户过滤：系统管理员可看全部，普通用户只能访问自己导出的记录和文件。
- 材料下载、模板管理员细粒度授权和审计日志仍需要后续继续补齐；涉及权限变更、删除和批量操作必须保留二次确认和审计预留。
...
- 模板版本不可变；导出记录必须绑定具体模板版本，确保导出文件可追溯。
...
- 草稿列表已复用模板管理的文种文件夹 -> 文种内卡片列表层级，接入草稿卡片、空状态、加载/错误状态、文种内新增/删除文种、文种内新建草稿和从草稿卡片进入工作台；草稿进入工作台后再选择同文种套版模板。导出记录已接入真实列表、权限过滤和历史文件下载；材料库和 AI 任务当前仍是预留入口，后续阶段接入真实列表、权限和操作；模板管理已接入文种卡片、新增/删除文种、模板卡片、新增模板、上传新版本和版本解析结果首版。
