...
- Agent P10D-G 前端格式 T13：已完成；后续只负责协助 T14/T15 消费前端格式状态。
- Agent P10D-H 导出 T14/T15：让导出和预览消费结构映射、节点正文和格式合并结果。
...
2. 推进 P10D T14/T15：让导出和预览消费结构映射、`DraftNode` 正文和格式合并结果，同时继续保留旧 `DraftBlock` fallback。
