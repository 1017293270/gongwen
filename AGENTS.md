...
2. P10 模板管理员后台基于当前模板版本/profile API 继续扩展上传、解析结果展示、字段映射和启停。
3. P8 后续可接入模板版本 `TemplateProfile` 做占位符填充值检查，但不要阻塞 P11 的导出前质检闭环。
4. P9 登录与基础权限仍是 MVP 闭环的关键后续，尤其是草稿、材料、模板、导出文件访问控制。
