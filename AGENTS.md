...
- Agent P10D-H 导出 T14/T15：T14 后端导出消费结构映射、节点正文和格式合并结果已完成；后续执行 T15 预览刷新和导出状态前端。
...
2. 推进 P10D T15：让工作台展示真实预览状态、刷新预览动作和后端导出阻断原因，同时继续保留旧 `DraftBlock` fallback。
