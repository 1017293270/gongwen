...
| --- | --- | --- | --- | --- | --- | --- | --- |
| T0 | 已完成 | Integration Agent | none | frozen contracts, migration sequence, API boundaries, 18081 baseline | `8f89c4e` | `git status --short --branch`; `Get-ChildItem backend/src/main/resources/db/migration \| Select-Object Name`; `Get-NetTCPConnection -LocalPort 18081` | no feature code changed; next agents must preserve frozen migration numbers |
| T1 | 已完成 | Backend Profile Agent | T0 | eastAsia/latin font split and structured line spacing baseline | `8f89c4e` | `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.template.profile.TemplateProfileParserTest" --tests "com.gongwen.assistant.template.profile.TemplateEffectiveFormattingServiceTest"` | full backend test not run per lightweight-test instruction; line spacing first pass covers AUTO/EXACT |
| T2 | 已完成 | Backend Profile Agent | T1 | document kind classifier hardens manual/template distinction | `8f89c4e` | `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.template.TemplateUploadServiceTest" --tests "com.gongwen.assistant.template.profile.TemplateProfileParserTest"` | full backend test not run per lightweight-test instruction; rule classifier first pass covers generated fixtures |
| T3 | 已完成 | Backend Structure Agent | T1, T2 | `DocumentStructureProfile` foundation | `8f89c4e` | `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.documentstructure.DocumentStructureExtractorTest" --tests "com.gongwen.assistant.template.TemplateUploadServiceTest"` | first pass derives nodes from existing `TemplateProfile.structures`; full OOXML fact extraction deferred |
| T4 | 已完成 | Backend Render Agent | T0 | LibreOffice render job backend | `8f89c4e` | `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.rendering.DocumentRenderPreviewServiceTest" --tests "com.gongwen.assistant.rendering.LibreOfficeRenderClientTest"` | local LibreOffice execution not run; render client command and service path/status behavior covered with test seam |
| T5 | 已完成 | Frontend Template Agent | T3, T4 | template parse workspace with structure and preview states | `8f89c4e` | `npm test -- src/App.test.tsx`; `npm run build`; `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.template.TemplateControllerTest" --tests "com.gongwen.assistant.template.TemplateUploadControllerTest"` | in-app browser tool unavailable; local Vite responded 200, dialog covered by App test |
| T6 | 已完成 | Backend Mapping Agent | T3 | structure mapping save/publish backend | `8f89c4e` | `.\gradlew.bat --no-daemon compileTestJava`; `.\gradlew.bat --no-daemon test --tests "com.gongwen.assistant.documentstructure.mapping.StructureMappingServiceTest" --tests "com.gongwen.assistant.documentstructure.mapping.StructureMappingControllerTest"` | first pass required slots are `TITLE` and `BODY`; richer official-document slot policy deferred |
| T7 | 已完成 | Frontend Template Agent | T5, T6 | mapping editor UI and publish flow | `8f89c4e` | `npm test -- src/App.test.tsx`; `npm run build`; `Invoke-WebRequest http://localhost:5173` | first pass uses inline role selects in parse workspace; complex bulk mapping UX deferred |
| T8 | 已完成 | Backend Draft Agent | T6 | persisted `DraftNode` model and API | `8f89c4e` | `.\gradlew.bat --no-daemon --console=plain compileTestJava`; `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.draft.node.DraftNodeServiceTest" --tests "com.gongwen.assistant.draft.node.DraftNodeControllerTest"` | plan script could not run because `.tools\gradle-8.10.2` is absent; wrapper with JDK 21 passed focused tests; frontend not touched |
| T9 | 已完成 | Frontend Workbench Agent | T8 | left structure tree and center structured editor bind to nodes | `8f89c4e` | `npm test -- src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | first pass keeps legacy `DraftBlock` materialization for AI/quality/export compatibility; node-aware AI/export deferred to T10/T14 |
| T10 | 已完成 | Backend AI Agent | T8 | node-aware AI operations | `8f89c4e` | `.\gradlew.bat --no-daemon --console=plain compileTestJava`; `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.ai.PromptBuilderTest" --tests "com.gongwen.assistant.ai.AiOutlineServiceTest" --tests "com.gongwen.assistant.ai.AiParagraphServiceTest" --tests "com.gongwen.assistant.ai.AiLocalOperationServiceTest"` | full backend test skipped per lightweight-test instruction; frontend routing still deferred to T11 |
| T11 | 已完成 | Frontend AI Agent | T9, T10 | right panel context follows selected node | `8f89c4e` | `npm test -- src/App.test.tsx`; `npm run build` | browser screenshot not captured because Browser/node_repl tooling unavailable; focused tests cover node action switching and node-metadata request routing |
...
| T16 | 已完成 | QA Agent | T1-T15 | fixture suite, regression tests, and docs sync | `d3683e0` | `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.template.profile.TemplateProfileParserTest"`; focused backend regression command for upload/structure/AI/format/export; `npm test -- src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | backend full test and full frontend `npm test --` skipped per lightweight-test instruction; Browser/node_repl automation still unavailable |
| T17 | 已完成 | Integration Agent | T1-T16 | final integration verification and progress closure | 未提交（T17 集成关闭） | `git status --short --branch`; `git diff --check`; migration numeric listing; API endpoint grep; focused backend regression commands from T16; `npm test -- src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | backend full test/full frontend test skipped per lightweight-test instruction; browser automation unavailable; next task should split frontend workbench/App boundaries |
...
| 2026-05-31 | T16 | Codex QA Agent | `d3683e0` | `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.template.profile.TemplateProfileParserTest"`; `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.template.TemplateUploadServiceTest" --tests "com.gongwen.assistant.documentstructure.DocumentStructureExtractorTest" --tests "com.gongwen.assistant.ai.AiOutlineServiceTest" --tests "com.gongwen.assistant.ai.AiParagraphServiceTest" --tests "com.gongwen.assistant.ai.AiLocalOperationServiceTest" --tests "com.gongwen.assistant.draft.node.DraftNodeFormatOverrideTest" --tests "com.gongwen.assistant.exporting.DraftWordExportServiceTest"`; `npm test -- src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | pass; fixture coverage now records placeholder, style, reference official document, manual/guide, complex table, header/footer, and missing font categories; focused regressions cover manual blocking, Chinese fonts, node AI, draft-local format overrides, and export trace snapshots | backend full test and full frontend `npm test --` skipped per lightweight-test instruction; browser automation remains unavailable |
| 2026-05-31 | T17 | Codex Integration Agent | 未提交（T17 集成关闭） | `git status --short --branch`; `git diff --check`; numeric migration listing; endpoint grep across `frontend/src` and `backend/src/main/java`; focused backend regression commands from T16; `npm test -- src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | pass with documented limits; branch is clean before T17 docs, migrations are sequential V1-V16 with only V12-V16 added in this plan, frontend API calls match backend route declarations, focused backend/frontend tests and build passed | backend full test/full frontend test skipped per lightweight-test instruction; browser automation unavailable because Browser tool was not exposed and node_repl failed |
...
- [x] Make export load published mapping for the draft template version.
- [x] Make export load `DraftNode` records where available, fallback to `DraftBlock` only during transition.
- [x] Merge draft node format overrides with template mapping/default formatting.
- [x] Block export when mapping is missing, document kind is disallowed, required slots are empty, or quality check blocks export.
- [x] Add export record fields for structure profile, mapping profile, formatting snapshot, and node snapshot.
- [x] Add tests proving draft-local font/line spacing does not change template defaults.
- [x] Run focused export and quality tests.
- [x] Update Progress Board row T14 and Progress Log.
...
- Export blockers: missing bound template version (`TEMPLATE_VERSION_REQUIRED`), template version not found/not ready, missing template profile, disallowed document kind (`DOCUMENT_KIND_EXPORT_BLOCKED`), missing published structure mapping (`STRUCTURE_MAPPING_REQUIRED`), published mapping missing required `TITLE`/`BODY` slots (`STRUCTURE_MAPPING_REQUIRED_SLOT_MISSING`), empty required export slot (`EXPORT_REQUIRED_SLOT_EMPTY`), missing latest quality check (`QUALITY_CHECK_REQUIRED`), latest quality check blocked (`QUALITY_CHECK_BLOCKED`), and unreadable template file (`TEMPLATE_FILE_UNAVAILABLE`).
- Export record trace fields: `structure_mapping_profile_id`, `structure_mapping_version`, `structure_profile_snapshot_json`, `mapping_profile_snapshot_json`, `formatting_snapshot_json`, and `node_snapshot_json`.
- Draft node behavior: export reads persisted `DraftNode` content before legacy `DraftBlock` blocks when nodes exist; legacy `DraftBlock` export remains the transition fallback.
- Formatting behavior: export merges draft node format overrides with template structure overrides and parsed effective formatting for this export request only; tests assert draft-local font and line spacing do not mutate the template default formatting object.
...
- [x] Review `git diff --stat` and confirm only expected files changed.
- [x] Check migrations are sequential and no old migration was edited.
- [x] Check backend API contracts match frontend types.
- [x] Run backend full test or document exact blocker.
- [x] Run frontend test/build or document exact blocker.
- [x] Use browser verification for workbench desktop layout and template parse workspace.
- [x] Update every Progress Board row to final status.
- [x] Add final Progress Log entry with full verification summary.
- [x] Update `docs/PROJECT_TASKS.md` current state and next recommended tasks.
- [x] Update `AGENTS.md` if the implementation changed APIs, env vars, data model, or workflow.
...
**T17 completion notes (2026-05-31):**

- Final remaining risks: full backend `scripts/backend-test.ps1` was not run because it depends on missing `.tools\gradle-8.10.2\bin\gradle.bat` and the user requested lighter tests; full frontend `npm test --` was skipped for the same lightweight-test direction; Browser verification was not captured because the in-app Browser tool was not exposed and node_repl failed to initialize.
- Final checks: working tree was clean before T17 docs, migrations are sequential V1-V16 with only V12-V16 added by P10D, API endpoint grep confirmed frontend calls have backend route declarations, focused backend regression commands passed, `npm test -- src/workbenchNodes.test.ts src/App.test.tsx` passed, and `npm run build` passed.
- Next single recommended task: split `frontend/src/App.tsx` workbench/template/export surfaces into feature components before adding the next UI-heavy P11/P9 slice.
