...
- Draft initialization uses each non-ignored mapped source node's own original text. Multiple body nodes never duplicate the first legacy `BODY_PARAGRAPH`, and `UNKNOWN` review nodes are preserved instead of being silently dropped.
...
- The center workbench preview is labelled as a structured editing preview. It follows persisted node source order and includes preserved static facts, but does not replace LibreOffice true preview as the pixel-level DOCX original.
...
- Creating or reinitializing a draft from this mapping produces one `DraftNode` per non-ignored mapped source node, each with its own source text.
...
| T27 | 已完成 | Integration Fix Agent | T21, T23, T24 | strict no-edit correspondence between source nodes, workbench preview, and export | 本提交（T27 修正） | `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.draft.node.DraftNodeServiceTest" --tests "com.gongwen.assistant.exporting.DraftWordExportServiceTest" --tests "com.gongwen.assistant.integration.DocxCompleteStructurePipelineTest"`; `npm test -- src/components/workbench/WorkbenchPreview.test.tsx src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | Existing drafts with stale partial-node snapshots must run “从原稿重建结构” after backend restart; true pixel-level verification still uses LibreOffice preview/export comparison; manual cases documented in `docs/DOCX_ROUNDTRIP_TEST_CASES.md` |
...
| 2026-05-31 | T27 | Codex Integration Fix Agent | 本提交（T27 修正） | `.\gradlew.bat --no-daemon --console=plain test --tests "com.gongwen.assistant.draft.node.DraftNodeServiceTest" --tests "com.gongwen.assistant.exporting.DraftWordExportServiceTest" --tests "com.gongwen.assistant.integration.DocxCompleteStructurePipelineTest"`; `npm test -- src/components/workbench/WorkbenchPreview.test.tsx src/workbenchNodes.test.ts src/App.test.tsx`; `npm run build` | pass; DraftNode 初始化 now preserves every non-ignored mapped source node, workbench structured preview renders persisted nodes in source order including static facts, default reinitialize overwrites stale nodes from the source document, and `docs/DOCX_ROUNDTRIP_TEST_CASES.md` documents repeatable automatic/manual checks | Existing browser session must refresh/reinitialize after backend restart; already exported old files with empty `export_strategy` are stale artifacts and should be regenerated |
