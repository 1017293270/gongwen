...
- 覆盖完整性不能依赖人工想到哪些控件就做哪些控件，必须用模板能力矩阵持续记录每个 Word 维度的解析、展示、风险、配置、质检和导出复现等级。
...
模板引擎必须用能力矩阵保证覆盖面，而不是只实现当前 UI 显示到的少数样式项。每个维度都要标记当前覆盖等级：

重要界面约束：能力矩阵是内部覆盖与 QA 机制，不是模板管理员主界面。管理员真正需要看到和编辑的是“识别到哪些公文结构、结构对应的 Word 原文是什么、每个结构有哪些可生效维度、这些维度如何影响工作台类 Word 预览和最终导出”。
...
| Word 维度 | 示例能力 | MVP 目标等级 | 说明 |
| --- | --- | --- | --- |
| 占位符 | 普通占位符、表格内占位符、跨 run 占位符 | L5 | 缺字段或跨 run 风险必须可见，导出前可阻断。 |
| 样式 | `styleId`、`styleName`、继承关系、候选语义块 | L4-L5 | 标题、正文、落款、日期等关键块必须可映射到样式。 |
| 段落版式 | 对齐、行距、首行缩进、段前段后 | L4-L5 | 标题居中、正文行距、落款右对齐等进入模板规则和导出计划。 |
| 字符版式 | 字体、字号、粗体、颜色 | L3-L5 | 关键样式从 Word 继承；管理员可覆盖少数公文关键规则。 |
| 分节与页面 | 纸张大小、方向、页边距、分节 | L1-L2 | 首版只读展示和风险提示，导出尽量继承原模板。 |
| 页眉页脚与页码 | 页眉、页脚、页码结构 | L1-L2 | 首版不编辑，必须提示复杂结构风险。 |
| 编号 | 多级编号、编号段落绑定、缩进 | L1-L2 | 首版不做编号编辑器，只识别和提示风险。 |
| 表格 | 行列、单元格、合并单元格、表格内占位符 | L2-L5 | 表格内占位符可填充；复杂表格结构先提示风险。 |
| 媒体与印章 | 图片、印章、浮动或嵌入锚点 | L1-L2 | 首版只读展示，不做拖拽排版。 |
| 未支持结构 | 域、脚注尾注、复杂 DrawingML、宏等 | L2 | 不静默忽略，进入 unsupported/risk 列表。 |

矩阵状态必须随着代码推进同步更新到任务表。新增解析能力时至少达到 L0-L2；新增可配置能力时必须说明能否进入 L4 质检和 L5 导出复现。
