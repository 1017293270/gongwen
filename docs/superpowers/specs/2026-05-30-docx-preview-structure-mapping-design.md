...
本节固化模板管理、起草工作台、格式编辑、AI 增强、真预览和导出的完整流程。核心边界是：

```text
模板管理 = 改模板结构、模板默认格式、模板静态内容
起草工作台 = 改当前草稿内容、当前草稿局部格式、当前草稿 AI 增强
LibreOffice 真预览 = 只负责高保真校验，不作为主编辑器
```

### 20.1 模板版本上传与解析流程

目标：把上传的 `.docx` 变成可解释、可映射、可预览、可复用的模板版本。
...
模板管理员上传 DOCX
  ...
  -> 创建 template_version
  -> 执行 OOXML 事实提取
  -> 生成 DocumentStructureProfile
  -> 执行 effective formatting 归一
  -> 执行文件类型识别
  -> 生成 DocumentKindAnalysis
  -> 执行结构语义建议
  -> 生成 StructureSuggestion
  -> 创建默认 StructureMappingProfile 草稿
  -> 创建 LibreOffice 渲染任务
  -> 前端进入文档解析工作台
```
...
- 文件类型：占位符模板、样式模板、范文、手册、普通 Word、混合文件。
- 结构树：body、paragraph、table、header、footer 等事实节点。
- 语义建议：标题、主送、正文、附件、落款、日期等。
- 版式摘要：中文字体、西文字体、字号、行距、缩进、对齐。
- 风险列表：文本框、图片、页眉页脚、复杂表格、字体缺失等。
- 真预览状态：生成中、成功、失败、字体缺失警告。

阻断规则：

- `MANUAL_OR_GUIDE`、`ORDINARY_DOCUMENT` 默认不进入套版模板发布。
- `UNKNOWN_DOCUMENT` 需要模板管理员确认文件用途。
- 存在 `BLOCKING` 风险时，不允许发布为自动套版模板。
- 渲染预览失败不一定阻断解析，但应标记为导出风险。

### 20.2 模板管理侧编辑流程

目标：模板管理员可以编辑模板结构、默认格式和静态内容，但不会影响已导出的历史文件。

模板侧允许编辑：

- 节点角色：标题、主送、正文、附件、落款、日期、忽略、保留。
- 槽位映射：某个节点对应 `title`、`recipient`、`body`、`signature` 等槽位。
- 模板默认格式：字体、字号、加粗、对齐、首行缩进、行距、段前段后。
- 模板静态内容：固定红头、固定说明、固定落款单位、固定附件标题等。
- 风险处理策略：保留、替换、忽略、阻断。
...
模板管理员打开模板解析工作台
  ...
  -> 保存为 mapping draft
  -> 系统执行模板级质检
  -> 无阻断风险后发布 mapping version
  -> 工作台后续绑定该模板版本时读取发布映射
```

版本规则：

- 原始 `template_version` 不可变。
- 每次发布结构映射生成新的 `structure_mapping_profile.version`。
- 如果修改模板静态内容会改变原始 DOCX，应生成新的模板版本，而不是覆盖旧版本。
- 如果只修改映射和默认格式，可生成新的 mapping version。
- 导出记录必须保存 `template_version_id` 和 `structure_mapping_profile_id`。

### 20.3 起草工作台初始化流程

目标：起草人打开工作台时，得到一个稳定的结构化草稿，而不是一整段不可控正文。
...
起草人选择文种
  -> 选择模板版本
  -> 后端读取已发布 StructureMappingProfile
  -> 生成或加载 Draft
  -> 生成或加载 DraftNode 列表
  -> 按模板节点角色组织左侧结构树
  -> 中间渲染结构化 Word 风格编辑视图
...
- 已有草稿：读取原有 `DraftNode` 内容、状态和格式覆盖。
- 新建草稿：按模板映射生成空节点。
- 模板未完成映射：提示“模板结构未确认”，允许临时起草，但导出前阻断。
- 文件类型不适合套版：阻断绑定或要求管理员处理。

### 20.4 起草工作台内容编辑流程

目标：起草人可以灵活改当前稿子内容，但不污染模板。

流程：

```text
用户在左侧或中间选中结构节点
  -> selectedWorkbenchNodeId 更新
  -> 中间编辑块获得焦点
  -> 右侧切换为该节点的 AI/质检/格式工具
  -> 用户编辑文本
  -> 前端标记节点 dirty
  -> 自动保存或手动保存 DraftNode
  -> 节点状态变为 USER_FILLED 或 USER_MODIFIED_AFTER_AI
  -> 质检状态变为 STALE
  -> 真预览状态变为 OUTDATED
```
...
- 编辑标题节点只改变当前草稿标题，不改变模板标题格式。
- 编辑正文节点只改变当前正文节点内容，不改变节点角色。
- 编辑主送、附件、落款、日期时，应保留其结构角色。
- 用户需要新增正文段落时，创建新的 `DraftNode`，而不是把多段塞进一个节点。
- 用户需要拆分正文时，应走“拆分节点”操作。
- 用户需要合并正文时，应走“合并节点”操作。
...
目标：起草人可以编辑当前草稿的字体、行距、居中等基础 Word 格式，但只影响当前草稿。

推荐第一期支持结构节点级格式编辑，不做任意字符级格式编辑。

支持字段：

- 中文字体 `eastAsiaFontFamily`
- 西文字体 `latinFontFamily`
- 字号 `fontSizePt`
- 加粗 `bold`
- 对齐 `alignment`
- 首行缩进 `firstLineIndent`
- 左右缩进 `leftIndent` / `rightIndent`
- 行距 `lineSpacing`
- 段前段后 `spaceBefore` / `spaceAfter`
- 是否保留同页 `keepWithNext` / `keepLines`
...
用户选中结构节点
  ...
  -> 面板显示合并后的当前格式
  -> 用户修改字体/字号/行距/居中等
  -> 前端立即用 CSS 模拟更新中间编辑视图
  -> 保存 DraftNodeFormatOverride
  -> 节点状态标记为 FORMAT_OVERRIDDEN
  -> 真预览状态变为 OUTDATED
  -> 后台按需重新生成 DOCX 和 LibreOffice 预览
```

格式合并优先级：

```text
当前草稿节点格式覆盖 DraftNodeFormatOverride
  > 模板结构槽位格式 StructureMapping formattingOverride
  > 原 DOCX effectiveFormatting
  > 文种默认公文格式
  > 系统默认格式
```
...
- 工作台格式修改只保存到当前草稿。
- 工作台不能直接修改模板默认格式。
- 模板管理员在模板管理中修改默认格式后，不自动覆盖起草人已做的草稿格式覆盖。
- 用户可以选择“恢复模板默认格式”，删除当前节点的草稿格式覆盖。
- 导出记录保存最终合并后的格式快照，确保追溯。

权限建议：

- 起草人可以改当前草稿节点格式。
- 模板管理员可以改模板默认格式。
- 系统管理员可以配置允许起草人修改哪些格式项。
- 涉及红头、版记、密级、页眉页脚等关键格式时，默认只允许模板管理员修改。

### 20.6 模板字体与格式编辑流程

目标：模板管理员可以维护模板默认格式，形成组织统一公文规范。

流程：

```text
模板管理员进入模板编辑器
  -> 选择结构槽位，例如标题/正文/一级标题/落款
  -> 编辑默认字体、字号、行距、对齐、缩进
  -> 保存为模板格式规则草稿
  -> 系统生成格式差异摘要
  -> 执行模板级质检
  -> 发布 mapping version 或新模板版本
```
...
- 新草稿默认使用最新发布的模板格式。
- 已有草稿保留自己的内容和草稿格式覆盖。
- 已有草稿如果未做格式覆盖，可提示“模板格式有更新，是否应用”。
- 历史导出不变。

### 20.7 AI 围绕结构节点工作的流程

目标：保留右侧现有 AI 能力，但让所有 AI 操作都绑定结构节点和影响范围。
...
用户选中已有正文节点
  ...
  -> 用户采纳
  -> 仅替换当前节点 content
  -> 节点状态变为 USER_MODIFIED_AFTER_AI 或 AI_GENERATED
```
...
- AI 不能隐式改变节点角色。
- AI 不能把主送、附件、落款、日期混入正文。
- AI 不能覆盖锁定节点。
- AI 建议失败不影响用户手动编辑。
- AI trace 不记录完整敏感正文，只记录摘要和节点标识。

### 20.8 LibreOffice 真预览刷新流程

目标：在不把 LibreOffice 当编辑器的前提下，提供高保真版式校验。
...
草稿内容或格式发生变化
  -> 当前结构化编辑视图立即更新
  -> 标记 renderPreviewStatus = OUTDATED
  -> 用户点击刷新预览，或系统 debounce 后触发
  -> 后端按模板和草稿节点生成临时 DOCX
  -> LibreOffice headless 转 PDF
  -> PDF 转 PNG pages
  -> 保存 DocumentRenderPreview
  -> 前端刷新真预览
```
...
- 用户输入过程中不每个按键都渲染。
- 保存草稿后可延迟触发预览任务。
- 导出前必须使用最新内容重新生成或校验预览。
- 渲染失败不丢失草稿内容，但导出前应提示风险。
- 字体缺失时预览可继续，但显示字体风险。
...
```text
CURRENT：真预览与当前草稿一致。
OUTDATED：草稿变化后尚未重新渲染。
RENDERING：正在渲染。
FAILED：渲染失败。
UNAVAILABLE：当前环境未配置 LibreOffice。
```

### 20.9 高亮定位与编辑层流程

目标：用户点击结构节点能在真预览里定位；点击预览高亮能回到结构节点；编辑仍落到结构化节点。
...
```text
LibreOffice 输出 PDF
  -> PDFBox 读取 text layer
  -> 后端用节点文本、位置、hash 匹配 PDF 坐标
  -> 生成 nodeCoordinateMap
  -> 前端在 PNG/PDF 上绘制透明高亮框
```
...
- 标题、主送、落款、日期等短文本可以后续支持预览上方小浮层编辑。
- 长正文不建议在图片上原地编辑，应跳回结构化编辑块。
- 浮层编辑保存后仍写入 `DraftNode`，不是修改 PNG/PDF。

### 20.10 导出流程

目标：导出使用同一套结构、内容和格式合并规则，确保可追溯。
...
用户点击导出
  ...
  -> 合并草稿格式覆盖与模板格式
  -> 执行导出前质检
  -> 若 exportBlocked=true，返回阻断原因和节点
  -> 若通过，生成 DOCX
  -> 可选：LibreOffice 生成导出预览
  -> 保存 ExportRecord
  -> 返回下载链接
```
...
- 模板管理中改默认字体、行距、静态内容，不直接改任何草稿。
- 工作台中改字体、行距、正文内容，不回写模板。
- 工作台可以读取模板默认格式，也可以覆盖当前草稿节点格式。
- 模板更新后，已有草稿是否应用更新必须由用户或策略确认。
- 导出永远记录实际使用的模板版本、映射版本和格式快照。
- 高级 Word 编辑器如果后续接入，只能作为“高级编辑入口”，不能绕过结构化保存和导出追溯。
...
这样可以先把项目最关键的产品体验跑通：用户保留公文结构，同时能用 AI 扩展内容，也能灵活调整当前稿子的基础字体和版式，最后用真预览和导出闭环校验。
