...
- P10B 结构维度闭环已形成首个后端共享合同：profile 输出可识别结构、原文片段、位置、来源和字体、字号、加粗、对齐、首行缩进、行距、段前段后，维度编辑保存到 `template_rule`，同一套生效格式已被工作台预览、基础质检和 `.docx` 导出复用。
...
状态：进行中。已完成首个切片：工作台可对当前草稿调用 `POST /api/exports/drafts/{draftId}/word`，按草稿绑定的模板版本生成并下载 `.docx`，并已复用 P10B 生效格式输出 Word 结果用于和实际模板效果对比。
