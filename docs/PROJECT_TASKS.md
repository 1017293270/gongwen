...
- P10B 结构维度闭环首个切片：profile 输出可识别结构、原文片段、位置、来源和字体、字号、加粗、对齐、首行缩进、行距、段前段后，模板解析弹窗改为“结构与维度”，维度编辑保存到 `template_rule`，工作台类 Word 预览按所选模板结构维度渲染。
- P10C 工作台结构节点前端切片：新增 `WorkbenchNode` 派生层，左栏正文目录、中间类 Word 纸张和右栏局部操作上下文统一围绕结构节点；正文小标题和正文内容分开展示、选择和编辑，底层仍兼容现有 `DraftBlock`。
- P11 导出体验首个切片：新增草稿绑定模板版本后的 Word 导出 API 和工作台右栏入口，导出当前草稿用于和真实 Word 结果对比。

当前推荐下一阶段：

- P11 导出体验增强，把 `exportBlocked` 接入导出前阻断，并补导出记录列表和历史文件下载。
- P10C 结构节点持久化，新增后端节点 API/存储，并把 AI/质检/导出从草稿块逐步迁到 `nodeId`。
- P10B 导出复现，把已落库的结构维度复用到质检和 `.docx` 导出。
...
- 将标题居中、正文行距、正文首行缩进、落款右对齐等已保存生效维度接入 P8/P11 质检和 `.docx` 导出复现。
...
状态：进行中。已完成首个切片：工作台可对当前草稿调用 `POST /api/exports/drafts/{draftId}/word`，按草稿绑定的模板版本生成并下载 `.docx`，用于和实际 Word 效果对比。
