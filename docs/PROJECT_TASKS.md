...
当前分支：`codex/p10d-docx-structure-workbench`
...
- P11 导出体验阻断与追溯切片：新增草稿绑定模板版本后的 Word 导出 API 和工作台右栏入口，导出前会自动保存当前草稿；基础质检保持独立入口，不作为导出前置动作，后端仅按模板版本、结构映射、文档类型和必填槽位等导出硬约束阻断；导出记录页已接入真实列表、详情弹窗、失败原因展示、失败重试和成功历史文件下载。
...
- P10D T15 工作台预览刷新与导出状态前端：右栏新增真实预览状态面板，覆盖当前、待刷新、生成中、失败和未选择模板；正文/格式/AI 采纳等修改会标记真实预览待刷新。模板解析工作台继续调用模板版本 render-preview API；起草工作台会先保存当前草稿，再调用草稿级 render-preview API，用当前草稿 DOCX 生成真实页面图；后端导出阻断消息继续在工作台导出状态和 Toast 中展示。
- P10D T15 草稿级真实预览修正：新增 `POST /api/drafts/{draftId}/render-preview` 和 `GET /api/drafts/{draftId}/render-preview`，预览记录可绑定 `draft_id`（Flyway V19），草稿预览和模板原稿预览隔离，工作台重新进入时优先读取草稿预览而不是模板快照，草稿预览不写入导出记录；删除持久化结构会把相关 `DraftNode` 标记为 `DELETED`，结构编辑、真实预览和 Word 导出均跳过该节点，原 DOCX 原位替换路径会移除被删除的正文段落。
- Outline-v2 提纲空结构兜底：DeepSeek 返回 JSON 但 `sections=[]` 时，后端不再直接报 `AI_RESPONSE_INVALID`，会按文种生成默认一级/二级正文结构，并把“AI 未返回可用提纲结构”写入缺失信息提示。
- Outline-v2 套版导出修正：应用新提纲时旧正文区节点保留为 `DELETED` 供 DOCX 原位替换链路删除，新 synthetic 提纲/正文节点携带原 DOCX 锚点和样式来源；真实预览和 Word 导出均先定位旧正文区原始段落、插入新节点，再删除旧模板正文区，避免应用提纲后仍渲染或导出原模板正文及后续示例标题。
- Outline-v2 格式来源修正：正文标题层级统一按原文编号识别，`一、`/`二、` 为一级，`（一）`/`（二）` 为二级，`1.`/`1、` 为三级。新 synthetic 标题节点优先按原模板节点文本编号找样式源；缺少对应 `BODY_HEADING_LEVEL_1/2/3` 时退到 `BODY` 或系统默认格式并提示格式依据不足，不再跨级借其他标题样式。历史节点已保存跨级 `styleSourceNodeKey` 时，工作台节点格式解析和 Word 导出读取阶段会纠偏，避免缩进和对齐在预览/导出间来回漂移。
- Outline-v2 DOCX 插入修正：真实预览和 Word 导出插入 synthetic 提纲/正文节点时，复制段落样式会清理未解析的自动编号 `numPr`、左缩进和悬挂缩进 `ind`，再由解析出的 formatting 显式写回 alignment、首行缩进、段前段后、行距和字体，避免三级标题等编号段落继承隐藏缩进后位置漂移。
- P10D/P10E 可编辑映射修正：模板解析工作台中，用户手动把节点映射为 `ISSUING_ORGAN`、`DOC_NUMBER`、`TABLE_ATTACHMENT` 等业务字段角色后，草稿节点初始化和工作台结构编辑会按可编辑节点处理；`STATIC_TEXT` 和 `IGNORE` 仍保持固定文本/忽略语义。
...
- 质检会生成 `exportBlocked` 供用户判断草稿风险，但不作为 Word 导出的强制前置动作。
- AI 建议首版只展示，不自动改正文。
...
- 已完成部门树管理 API/UI：查询、新增、编辑、删除；部门删除仅允许没有子部门、账号或业务数据引用的空部门，前端二次确认，后端物理删除；部门编号由后端按层级自动生成，前端不暴露编号输入和编号列，部门列表使用统一管理列表组件。
- 已完成账号管理 API/UI：查询、新增、编辑、重置密码、停用；账号新增/编辑使用全局弹窗，账号列表与部门列表复用统一管理列表组件。
- 已完成文种 CRUD API，并将前端新增/删除文种入口下沉到草稿列表和模板管理的文种文件夹页；删除入口位于文种卡片右上角并二次确认，侧边栏不再保留独立文种管理模块。
...
- 已验证草稿和文种服务会携带当前用户归属。
- 已验证草稿列表、模板管理中的新增/删除文种流程，以及部门管理和账号管理新增流程。
...
- 模板版本详情页和历史版本列表。
- 模板管理员权限接入。
- 模板删除入口已提供二次确认；已被草稿或草稿结构节点引用的模板不得直接删除，后端返回 `TEMPLATE_IN_USE_BY_DRAFTS` 业务错误并提示先删除相关草稿或更换草稿模板。后续批量操作仍必须先补确认、审计和权限。
- API 错误体验约定：后端所有 API 异常必须统一返回 `ApiResponse.error(errorCode, message)` JSON；数据库关联冲突、参数错误、上传过大、未知异常等要转成用户可读提示，不向前端泄露堆栈、SQL 或内部路径；前端 API 客户端需把后端 `message`、非 JSON 响应和网络失败统一转成可展示的错误文案。

验收标准：

- 管理员能按文种进入模板列表。
- 管理员能在模板管理的文种文件夹页新增和删除文种，不需要进入独立文种管理模块。
...
目标：让工作台节点格式面板回填当前模板/原 DOCX 的生效样式，并把“按提纲生成正文”从直接覆盖正文改为候选确认流程。

已完成范围：

- 后端 `DraftNode` 返回 `baseFormatting` 和 `effectiveFormatting`，前端节点格式面板默认展示当前生效样式，保存时只写草稿节点 `formatOverride`。
- AI 正文候选已持久化到草稿维度，支持创建、列表、编辑、丢弃、重试、逐段确认和批量确认；确认前不写入 `DraftNode` 或兼容 `DraftBlock`。
- 右栏新增正文候选画布，按提纲批量生成全部段落候选，展示生成中、失败、停止、重试、编辑、确认和批量确认状态；确认后刷新本地节点并标记真实预览待刷新。
- Flyway 迁移已到 `V21`：`V20__ai_paragraph_candidate.sql` 保持候选表基线，`V21__paragraph_candidate_node_draft_constraint.sql` 追加候选节点同草稿约束，避免修改已应用迁移。

后续范围：

- DeepSeek 真实 token 级 SSE 流式输出仍待接入；当前批处理事件已能渐进更新候选状态。
- 候选长期保留、清理策略和更完整的浏览器人工验收留待后续迭代。
...
状态：进行中。已完成导出前结构阻断与历史追溯切片：工作台可对当前草稿调用 `POST /api/exports/drafts/{draftId}/word`，按草稿绑定的模板版本生成并下载 `.docx`；点击导出会先保存当前草稿，再直接调用导出 API。基础质检由用户独立触发，后端导出服务不再读取最近一次质检结果；导出仍会按模板版本、结构映射、文档类型、必填结构槽位和文件生成错误阻断。导出成功会写入本地导出文件存储和 `export_record` 追溯字段，导出记录页可查看成功/失败记录、打开详情、重新下载成功历史文件，并对绑定草稿的失败记录发起重试。
...
- 未通过基础检查时阻止导出。
- 导出失败有可理解错误。
- 历史导出记录可追溯模板版本并下载成功文件。
- 失败记录可查看错误码和错误信息，绑定草稿的记录可重新触发导出。
- 普通用户只能查看和下载自己的导出记录，系统管理员可查看全部记录。
...
- Agent A 后端 P11：导出服务保持与基础质检解耦，只处理模板版本、结构映射、文档类型、必填槽位和文件生成错误；补导出记录模板版本追溯和稳定错误 shape。
- Agent B 前端 P11：工作台导出入口与基础质检入口分离，导出只展示导出中、导出失败和成功下载状态，质检状态留在基础质检面板。
