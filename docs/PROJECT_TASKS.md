...
- P10D T15 草稿级真实预览修正：新增 `POST /api/drafts/{draftId}/render-preview` 和 `GET /api/drafts/{draftId}/render-preview`，预览记录可绑定 `draft_id`（Flyway V19），草稿预览和模板原稿预览隔离，草稿预览不写入导出记录。
